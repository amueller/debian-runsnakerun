...
class MeliaeAdapter(squaremap.DefaultAdapter):
    ...
    def value(self, node, parent=None):
        ...
        try:
            ...
            contribution = int(node.get('totsize', 0) /
                               float(len(node.get('parents', ())) or 1))
    ...
    def label(self, node):
        ...
        if node.get('name'):
            ...
            result.append(text_type(node['value'])[:32])
    ...
    def filename(self, node):
        if 'module' in node and 'filename' not in node:
            try:
