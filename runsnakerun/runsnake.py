...
def mem_name(x):
    ...
    value = x.get('value')
    if value:
        if isinstance(value, string_types) and len(value) > MAX_NAME_LEN:
...
#        self.Bind(wx.EVT_TOOL, self.OnOpenFile, id=ID_OPEN)
        ...
        if not osx:
            tb.AddSeparator()
        # TODO: figure out why the control is sizing the label incorrectly on
        # Linux
        ...
        wx.EVT_CHOICE(self.viewTypeTool,
                      self.viewTypeTool.GetId(), self.OnViewTypeTool)
