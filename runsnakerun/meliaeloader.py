...
def promote_loops(loops, index, shared):
    ...
    for loop in loops:
        ...
        if external_parents:
            ...
            # each referent to loop holds a single reference to the loop rather
            # than many to children
...
def children(record, index, key='refs', stop_types=STOP_TYPES):
    ...
    for ref in record.get(key, []):
        try:
            ...
            # print 'No record for %s address %s in %s'%(key, ref,
            # record['address'] )
            # happens when an unreachable references a reachable that has been
            # compressed out...
            pass
        else:
...
def recurse_module(overall_record, index, shared, stop_types=STOP_TYPES,
                   already_seen=None, min_size=0):
    ...
    for record in recurse(
        ...
        if not record['refs']:
            ...
            # TODO: provide a flag to coalesce based on e.g. type at each level
            # or throughout...
...
def simplify_dicts(index, shared, simplify_dicts=SIMPLIFY_DICTS,
                   always_compress=ALWAYS_COMPRESS_DICTS):
    ...
    for to_simplify in iterindex(index):
        if to_simplify['address'] in to_delete:
            continue
        if to_simplify['type'] in simplify_dicts and 'compressed' not in to_simplify:
            ...
            for ref in refs:
                ...
                if child is not None and child['type'] == 'dict':
                    ...
                    if len(child_referrers) == 1 or to_simplify['type'] in always_compress:
                        ...
                        # rewrite anything *else* that was pointing to child to
                        # point to us...
                        ...
                        # now rewrite grandchildren to point to root obj
                        # instead of dict
...
#    print '%s bytes are unreachable from modules'%( unreachable )
    ...
    if include_interpreter:
        # Meliae produces quite a few of these un-referenced records, they
        # aren't normally useful AFAICS
...
def find_roots(disconnected, index, shared):
    ...
    """
    log.warn('%s disconnected objects in %s total objects',
             len(disconnected), len(index))
    natural_roots = [x for x in disconnected if x.get(
        'refs') and not x.get('parents')]
